...
class Cursor(object):
    ...
    def __init__(self, dictionary=False, **options):
        ...
        try:
            ...
            # Add SSL parameters for Supabase compatibility
            ssl_options = {
                'sslmode': 'require',
            ...
            try:
                ...
                # Fallback without SSL if SSL connection fails
                options_no_ssl = {k: v for k, v in options.items() if k not in ssl_options}
                conn = psycopg2.connect(**options_no_ssl)
