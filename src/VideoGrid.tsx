...
function VideoCardTile({ v, i }: { v: GridVideo; i: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="v3d-thumb relative w-full aspect-video overflow-hidden bg-muted/30 rounded-none sm:rounded-2xl">
          {v.thumb_url ? (
            <img
              src={v.thumb_url}
              alt={v.title}
          ...
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-background/60 via-transparent to-transparent opacity-0 sm:group-hover:opacity-100 transition-opacity" />
          {fmtDuration(v.duration_seconds) && (
            <span className="absolute bottom-2 right-2 rounded-md bg-background/85 px-1.5 py-0.5 text-[11px] font-medium text-foreground">
              {fmtDuration(v.duration_seconds)}
            </span>
          )}
        </div>
        <div className="flex items-start gap-3 mt-2.5 px-3 sm:px-0 w-full">
          <div className="h-9 w-9 shrink-0 rounded-full bg-muted overflow-hidden">
            {v.ownerAvatar ? (
              <img src={v.ownerAvatar} alt="" loading="lazy" decoding="async" width={36} height={36} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} className="h-full w-full object-cover" />
            ) : (
              <div className="flex h-full w-full items-center justify-center bg-muted text-[10px] font-bold text-foreground">
                {(v.ownerName || '?').slice(0, 2).toUpperCase()}
              </div>
            )}
          </div>
          <div className="min-w-0 flex-1">
            <h3 className="line-clamp-2 text-[14px] font-medium leading-[20px] text-foreground">
              {v.title}
            </h3>
            <div className="mt-0.5 flex flex-wrap items-center gap-x-1 text-[12px] leading-[16px] text-muted-foreground">
