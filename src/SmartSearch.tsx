...
export function SmartSearch() {
  ...
  /** Options currently reachable with the arrow keys. */
  const options: Array<{ key: string; kind: 'video' | 'term'; id: string }> = q.trim()
    ? results.map((r) => ({ key: `sr-${r.id}`, kind: 'video' as const, id: r.id }))
    : [...recent, ...TRENDING].map((t) => ({ key: `st-${t}`, kind: 'term' as const, id: t }));

  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Escape') { e.preventDefault(); setOpen(false); setActiveIndex(-1); return; }
    if (e.key === 'ArrowDown') {
      e.preventDefault();
    ...
    if (e.key === 'Enter') {
      ...
      if (opt?.kind === 'video') openVideo(opt.id);
      else if (opt?.kind === 'term') go(opt.id);
      else go(q);
    }
  };

  const activeId = activeIndex >= 0 ? options[activeIndex]?.key : undefined;
      ...
      {open && (
        <div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-md flex items-start justify-center pt-16 px-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-2xl glass-strong rounded-2xl border border-border/40 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-border/30">
              ...
              {q && (
                <button
                  ...
                  className="p-1.5 rounded-md text-muted-foreground hover:bg-muted/40 hover:text-foreground"
                >
            ...
            <div className="max-h-[60vh] overflow-y-auto" id="smart-search-listbox" role="listbox">
              {/* Live results */}
              {q.trim() && (
                <div className="p-3">
                  {loading && results.length === 0 && <SearchResultSkeleton rows={4} />}
                  {results.length === 0 && !loading && (
                    <p className="text-xs text-muted-foreground px-3 py-6 text-center">No matches yet — keep typing.</p>
                  )}
                  {results.map((r, i) => (
                    <button key={r.id} id={`sr-${r.id}`} role="option" aria-selected={activeIndex === i}
                      onMouseEnter={() => setActiveIndex(i)}
                      onClick={() => openVideo(r.id)}
                      className={`w-full flex items-center gap-3 p-2 rounded-lg text-left transition ${activeIndex === i ? 'bg-muted/40 ring-1 ring-primary/40' : 'hover:bg-muted/30'}`}>
                  ...
                  {recent.length > 0 && (
                    <section>
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-muted-foreground">
                          <Clock className="w-3 h-3" /> Recent
                        </h4>
                        <button
                          type="button"
                          onClick={() => { clearRecent(); setRecent([]); setActiveIndex(-1); }}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {recent.map((r, i) => (
                          <button key={r} id={`st-${r}`} role="option" aria-selected={activeIndex === i}
                            onMouseEnter={() => setActiveIndex(i)}
                            onClick={() => go(r)}
                            className={`glass px-3 py-1 rounded-full text-xs transition ${activeIndex === i ? 'border-primary/60 text-primary' : 'hover:border-primary/50 hover:text-primary'}`}>{r}</button>
                        ))}
                      </div>
                    </section>
                  )}
                  <section>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {TRENDING.map((t, i) => {
                        ...
                        return (
                          <button key={t} id={`st-${t}`} role="option" aria-selected={activeIndex === idx}
                            onMouseEnter={() => setActiveIndex(idx)}
                            onClick={() => go(t)}
                            className={`glass px-3 py-1 rounded-full text-xs transition ${activeIndex === idx ? 'border-secondary/60 text-secondary' : 'hover:border-secondary/50 hover:text-secondary'}`}>#{t}</button>
                        );
                      })}
                    </div>
                  </section>
