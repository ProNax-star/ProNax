...
/** Per-ad-view payout used for the earn toast copy. */
const AD_VIEW_PAYOUT = 0.001;
...
export default function Watch() {
  ...
  return (
    <div className="shrink-0 min-h-0 md:h-[calc(100dvh-3.25rem)] md:max-h-[calc(100dvh-3.25rem)] md:overflow-hidden flex flex-col bg-background px-0 sm:px-4">
      <div className={theater ? 'w-full h-full min-h-0 overflow-y-auto scrollbar-thin max-w-[1700px] mx-auto px-0 sm:px-2 py-0 sm:py-2' : 'w-full md:h-full min-h-0 max-w-[1600px] mx-auto px-0 lg:px-2 py-0 sm:py-1 md:grid md:grid-cols-12 md:gap-6 md:items-stretch md:overflow-hidden'}>
        {/* ============= Left Column: Video + Info + Comments (Independent Scroll Panel) ============= */}
        <div className="min-w-0 min-h-0 flex flex-col md:col-span-8 lg:col-span-8 md:h-full md:overflow-y-auto overscroll-contain pr-0 md:pr-2 scrollbar-thin space-y-3.5 scroll-gpu">
          ...
          <div className="relative md:rounded-2xl overflow-hidden bg-black aspect-video w-full max-h-[56.25vw] md:max-h-[580px] shrink-0 shadow-2xl sticky top-0 z-30 md:relative">
            ...
            <AnimatePresence>
              ...
              {!adActive && adRewarded && (
                ...
                >
                  <Check className="w-3.5 h-3.5" /> +{formatMoney(AD_VIEW_PAYOUT, { currency: DEFAULT_CURRENCY, precise: true })} earned
