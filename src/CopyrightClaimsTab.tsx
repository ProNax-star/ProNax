...
import {
  evaluateCopyrightDisputeWithAI,
  isCompleteCopyrightEvaluation,
  AI_UNAVAILABLE,
...
export function CopyrightClaimsTab() {
  ...
  // Set only when a real AI evaluation attempt fails; never a substitute result.
  ...
  /**
   * AI dispute merit evaluator.
   *
   * Returns `null` when the model is unreachable or returns an incomplete
   * payload. There is deliberately no local rule-engine fallback: a fabricated
   * merit score is indistinguishable from a real one to the moderator reading
   * it, and it would silently drive claim releases and revenue routing.
   */
  const handleEvaluateDisputeWithAI = async (
    ...
    try {
      const res = await evaluateCopyrightDisputeWithAI({
        claimId: claim.id,
        ...
        durationSec,
        disputeReason: claim.dispute_reason,
        claimant: claim.claimant,
      });

      if (!res.ok) {
        setEvalError(res.reason);
        toast.error(res.reason);
        return null;
      }

      if (!isCompleteCopyrightEvaluation(res.data)) {
        const reason = `${AI_UNAVAILABLE} (incomplete evaluation returned)`;
        setEvalError(reason);
        toast.error(reason);
        return null;
      }

      const evaluation = res.data as AIEvaluation;
  ...
  const handleBulkAIAutoResolve = async () => {
    ...
    toast.info(`Running AI auto-resolution queue for ${ids.length} claims...`);

    for (const id of ids) {
      const claim = claims.find((c) => c.id === id);
      if (!claim) continue;

      const evalObj = claim.ai_evaluation ?? (await handleEvaluateDisputeWithAI(claim));

      // No evaluation means no decision: leave the claim untouched rather than
      // acting on a guessed merit score.
      if (!evalObj) {
        skipped++;
        continue;
      }
      ...
      if (evalObj.recommendation === 'release_claim' || evalObj.merit_score >= 70) {
        await handleUpdateClaimStatus(id, 'released', 'Auto-released by AI evaluation queue');
        autoReleased++;
      } else {
    ...
    toast.success('Bulk AI auto-resolution complete', {
      description:
        `Released: ${autoReleased} | Muted: ${autoMuted}` +
        (skipped ? ` | Skipped (analysis unavailable): ${skipped}` : ''),
    });
    setSelectedIds(new Set());
    setBatchProcessing(false);
  };
            ...
            >
              ...
              <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
                ...
                <div className="p-4 rounded-xl bg-gradient-to-r from-purple-950/40 via-zinc-900 to-black border border-purple-500/40 space-y-4">
                  ...
                  {evalError && !evaluatingAI && (
                    <div
                      role="status"
                      className="mb-3 p-3 rounded-xl bg-amber-950/30 border border-amber-500/40 text-[11px] text-amber-300 flex items-center justify-between gap-3"
                    >
                      <span>{evalError}</span>
                      <button
                        type="button"
                        onClick={() => handleEvaluateDisputeWithAI(selectedClaim)}
                        className="shrink-0 px-2 py-1 rounded border border-amber-500/40 text-amber-200 hover:bg-amber-500/10"
                      >
                        Retry
                      </button>
                    </div>
