...
// Bypass keys (API keys that bypass rate limiting)
const BYPASS_KEYS = new Set<string>(
  process.env.RATE_LIMIT_BYPASS_KEY?.split(',').filter(Boolean) || []
);
...
function hasBypass(identifier: string): boolean {
  return BYPASS_KEYS.has(identifier);
}
 ...
 * @param identifier - IP, user ID, or API key to whitelist
 */
export function whitelistIdentifier(identifier: string): void {
  BYPASS_KEYS.add(identifier);
  console.log(`Whitelisted identifier: ${identifier}`);
}

/**
 * Remove an identifier from whitelist
 * @param identifier - IP, user ID, or API key to remove
 */
export function removeWhitelist(identifier: string): void {
  BYPASS_KEYS.delete(identifier);
 ...
 */
export function getWhitelistedIdentifiers(): string[] {
  return Array.from(BYPASS_KEYS);
}
