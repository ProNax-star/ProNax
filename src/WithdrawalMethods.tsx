...
import { Banknote, Plus, Trash2, CheckCircle2, Smartphone, Mail, Coins, ShieldAlert, ShieldCheck } from 'lucide-react';
...
export default function WithdrawalMethods({ userId, onChange }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, user_id, method_type, account_identifier, account_holder_name, is_default, updated_at, verification_status, verified_at')
      .eq('user_id', userId)
  ...
  const startVerification = async (id: string) => {
    setBusy(true);
    const { error } = await supabase.rpc('start_withdrawal_method_verification', { p_method_id: id });
    setBusy(false);
    if (error) { toast.error(error.message); return; }
    setVerifyingId(id);
    setCode('');
    toast.success('Verification code sent', { description: 'Check your ProNax notifications for the 6-digit code.' });
    load();
  };
  ...
  const confirmVerification = async (id: string) => {
    ...
    setBusy(true);
    const { error } = await supabase.rpc('confirm_withdrawal_method_verification', { p_method_id: id, p_code: code.trim() });
    setBusy(false);
    if (error) { toast.error(error.message); return; }
    setVerifyingId(null);
    setCode('');
    ...
    >
      ...
      <div className="border-t border-border/30 pt-3">
        ...
        {loading ? (
          ...
          <p className="text-xs text-muted-foreground py-4 text-center">No payout accounts linked yet. Link and verify at least one to enable withdrawals.</p>
        ) : (
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                        ...
                        {m.verification_status === 'verified' ? (
                          <span className="inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-full border border-emerald-500/30 text-emerald-300">
                            <CheckCircle2 className="w-2.5 h-2.5" /> Verified
                          </span>
                        ) : m.verification_status === 'pending' ? (
                          <span className="inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-full border border-amber-500/30 text-amber-300">
                            <ShieldCheck className="w-2.5 h-2.5" /> Verification pending
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded-full border border-destructive/30 text-destructive">
                    ...
                    verifyingId === m.id ? (
                      <div className="flex items-center gap-1">
                        <input
                          ...
                          placeholder="000000"
                          inputMode="numeric"
                          className="w-20 bg-muted/30 border border-primary/40 rounded-md px-2 py-1 text-xs font-mono tabular-nums"
                        />
                        <button
                          onClick={() => confirmVerification(m.id)}
                          disabled={busy}
                          className="px-2.5 py-1 rounded-md gradient-primary text-primary-foreground text-[10px] font-semibold disabled:opacity-60"
                        >
                          Confirm
                        </button>
                      </div>
                      ...
                      <button
                        onClick={() => startVerification(m.id)}
                        disabled={busy}
                        className="px-2.5 py-1 rounded-md glass border border-primary/40 text-primary text-[10px] font-semibold disabled:opacity-60"
                      >
                        {m.verification_status === 'pending' ? 'Enter code' : 'Verify'}
                      </button>
                    )
                  )}
                  <button
