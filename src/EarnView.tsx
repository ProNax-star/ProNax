...
/** Payout threshold and average member value — display constants, not fabricated balances. */
const PAYOUT_THRESHOLD = 100;
const AVG_MEMBERSHIP_VALUE = 3.38;
...
export const EarnView: React.FC<EarnViewProps> = ({ channelStats }) => {
  ...
  useEffect(() => {
    ...
    // Estimate membership count from revenue using the configured average member value.
    const estimatedMembershipCount =
      membershipRevenue > 0 && AVG_MEMBERSHIP_VALUE > 0
        ? Math.round(membershipRevenue / AVG_MEMBERSHIP_VALUE)
        : 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <div className="p-4 bg-[#262626] rounded-xl border border-[#333]">
            ...
            <p className="text-xl font-bold text-white mt-1">
              {formatMoney(walletData.balance, { currency: DEFAULT_CURRENCY })}
            </p>
            <span className="text-[11px] text-emerald-400 mt-1 block">
              {walletData.balance >= PAYOUT_THRESHOLD
                ? `Threshold met (${formatMoney(PAYOUT_THRESHOLD, { currency: DEFAULT_CURRENCY })} min)`
                : `Below ${formatMoney(PAYOUT_THRESHOLD, { currency: DEFAULT_CURRENCY })} threshold`}
            </span>
          </div>
